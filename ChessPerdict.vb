...
' * Predict Not Working************************************************************************(+)
' * Chess Predict Taking AAllDraw Lot Of Time********************************************************RS*****0.12**4**Managements and Cuation Programing**(+)
...
Namespace Refrigtz
    Class ChessPerdict
        ...
        Public AAllDraw As List(Of AllDraw) = Nothing
        ...
        Public Sub InitiateForEveryKindThingHome(DummyHA As AllDraw, ii As Integer, jj As Integer, a As Color, Table As Integer(,), Order As Integer,
            ...
            If Order = 1 Then
                ...
                While i < AllDraw.SodierMidle
                    Try
                        ...
                        Dummy.SolderesOnTable(i) = New DrawSoldier(ii, jj, a, Table, Order, False,
                            i)
                        ...
                        While j < AllDraw.SodierMovments
                            ...
                            Dummy.SolderesOnTable(i).SoldierThinking(j).t = New Thread(AddressOf Dummy.SolderesOnTable(i).SoldierThinking(j).Thinking)
                ...
                While i < AllDraw.ElefantMidle
                    Try
                        ...
                        Dummy.ElephantOnTable(i) = New DrawElefant(ii, jj, a, Table, Order, False,
                            i)
                        'For All Possible Movments.
                        j = 0
                        While j < AllDraw.ElefantMovments
                            ...
                            Dummy.ElephantOnTable(i).ElefantThinking(j).t = New Thread(AddressOf Dummy.ElephantOnTable(i).ElefantThinking(j).Thinking)
                ...
                While i < AllDraw.HourseMidle
                    Try
                        ...
                        Dummy.HoursesOnTable(i) = New DrawHourse(ii, jj, a, Table, Order, False,
                            i)
                        'For All Possible Movments.
                        j = 0
                        While j < AllDraw.HourseMovments
                            ...
                            Dummy.HoursesOnTable(i).HourseThinking(j).t = New Thread(AddressOf Dummy.HoursesOnTable(i).HourseThinking(j).Thinking)
                ...
                While i < AllDraw.BridgeMidle
                    Try
                        ...
                        Dummy.BridgesOnTable(i) = New DrawBridge(ii, jj, a, Table, Order, False,
                            i)
                        ...
                        While j < 16
                            ...
                            Dummy.BridgesOnTable(i).BridgeThinking(j).t = New Thread(AddressOf Dummy.BridgesOnTable(i).BridgeThinking(j).Thinking)
                ...
                While i < AllDraw.MinisterMidle
                    Try
                        ...
                        Dummy.MinisterOnTable(i) = New DrawMinister(ii, jj, a, Table, Order, False,
                            i)
                        'For All Possible Movments.
                        j = 0
                        While j < AllDraw.MinisterMovments
                            ...
                            Dummy.MinisterOnTable(i).MinisterThinking(j).t = New Thread(AddressOf Dummy.MinisterOnTable(i).MinisterThinking(j).Thinking)
                ...
                While i < AllDraw.KingMidle
                    Try
                        ...
                        Dummy.KingOnTable(i) = New DrawKing(ii, jj, a, Table, Order, False,
                            i)
                        ...
                        While j < AllDraw.KingMovments
                            ...
                            Dummy.KingOnTable(i).KingThinking(j).t = New Thread(AddressOf Dummy.KingOnTable(i).KingThinking(j).Thinking)
            ...
            Else
                ...
                While i < AllDraw.SodierHigh
                    ...
                    i += 1
                End While
                'For All Brwon elepahnt Objects.
                ...
                While i < AllDraw.HourseHight
                    Try
                        ...
                        j = 0
                        While j < AllDraw.HourseMovments
                            'Thinking of Brwon Hourse Operations.
                            Dummy.HoursesOnTable(i).HourseThinking(j).Table = HoursesOnTable(i).HourseThinking(j).Table
                            Dummy.HoursesOnTable(i).HourseThinking(j).ThinkingBegin = True
                            Dummy.HoursesOnTable(i).HourseThinking(j).ThinkingFinished = False
                            Dummy.HoursesOnTable(i).HourseThinking(j).t = New Thread(AddressOf Dummy.HoursesOnTable(i).HourseThinking(j).Thinking)
                ...
                'For All Bridesg Brwon Objects.
                i = AllDraw.BridgeMidle
                While i < AllDraw.BridgeHigh
                    Try
                        'When Brwon Bridges Non Existing Continue Traversal Back.
                        If BridgesOnTable(i) Is Nothing Then
                            i += 1
                            Continue While
                        End If
                        ...
                        ii = CType(BridgesOnTable(i).Row, Integer)
                        jj = CType(BridgesOnTable(i).Column, Integer)
                        'Construction of Brwon Bridges With Local Variables.
                        Dummy.BridgesOnTable(i) = New DrawBridge(ii, jj, a, Table, Order, False,
                            i)
                        'For All Possible Bridges Movments.
                        j = 0
                        While j < AllDraw.BridgeMovments
                            'Thinking of Brwon Bridges Operations.
                            Dummy.BridgesOnTable(i).BridgeThinking(j).Table = BridgesOnTable(i).BridgeThinking(j).Table
                            Dummy.BridgesOnTable(i).BridgeThinking(j).ThinkingBegin = True
                            Dummy.BridgesOnTable(i).BridgeThinking(j).ThinkingFinished = False
                            Dummy.BridgesOnTable(i).BridgeThinking(j).t = New Thread(AddressOf Dummy.BridgesOnTable(i).BridgeThinking(j).Thinking)
                ...
                While i < AllDraw.MinisterHigh
                    Try
                        'When Brwon Minister Non Existing Continue Traversal Back.
                        If MinisterOnTable(i) Is Nothing Then
                            i += 1
                            Continue While
                        End If
                            ...
                            If System.Math.Abs(Me.TableList(index)(Row, Column)) = 1 Then
                                ...
                                If a = Color.Gray Then
                                    Try
                                        ...
                                        SolderesOnTable(So1) = New DrawSoldier(Row, Column, a, Me.TableList(index), 1, False,
                                            So1)
                                        'Increase So1.
                                        So1 += 1
                                ...
                                Else
                                    ...
                                    Try
                                        ...
                                        SolderesOnTable(So2) = New DrawSoldier(Row, Column, a, Me.TableList(index), -1, False,
                                            So2)
                                        'Increase So2.
                                        So2 += 1
                            ...
                            ElseIf System.Math.Abs(Me.TableList(index)(Row, Column)) = 2 Then
                                ...
                                If a = Color.Gray Then
                                    Try
                                        ...
                                        ElephantOnTable(El1) = New DrawElefant(Row, Column, a, Me.TableList(index), 1, False,
                                            El1)
                                ...
                                Else
                                    ...
                                    Try
                                        ...
                                        ElephantOnTable(El2) = New DrawElefant(Row, Column, a, Me.TableList(index), -1, False,
                                            El2)
                                        'Increament of Index.
                                        El2 += 1
                            ...
                            ElseIf System.Math.Abs(Me.TableList(index)(Row, Column)) = 3 Then
                                ...
                                If a = Color.Gray Then
                                    ...
                                    Try
                                        ...
                                        Catch t As Exception
                                            Log(t)
                                        End Try
                                        'Construction of Draw Brwon Hourse.
                                        HoursesOnTable(Ho1) = New DrawHourse(Row, Column, a, Me.TableList(index), 1, False,
                                            Ho1)
                                        'Increament of Index.
                                        Ho1 += 1
                                ...
                                Else
                                    ...
                                    Try
                                        ...
                                        HoursesOnTable(Ho2) = New DrawHourse(Row, Column, a, Me.TableList(index), -1, False,
                                            Ho2)
                                        'Increament of Index.
                                        Ho2 += 1
                            ...
                            ElseIf System.Math.Abs(Me.TableList(index)(Row, Column)) = 4 Then
                                ...
                                If a = Color.Gray Then
                                    ...
                                    Try
                                        ...
                                        BridgesOnTable(Br1) = New DrawBridge(Row, Column, a, Me.TableList(index), 1, False,
                                            Br1)
                                        'Increamnt of Index.
                                        Br1 += 1
                                ...
                                Else
                                    ...
                                    Try
                                        ...
                                        BridgesOnTable(Br2) = New DrawBridge(Row, Column, a, Me.TableList(index), -1, False,
                                            Br2)
                                        'Increament of Index.
                                        Br2 += 1
                                        'wehn Brwon New Bridges Detected Increament Max Index.
                            ...
                            ElseIf System.Math.Abs(Me.TableList(index)(Row, Column)) = 5 Then
                                ...
                                If a = Color.Gray Then
                                    ...
                                    Try
                                        ...
                                        MinisterOnTable(Mi1) = New DrawMinister(Row, Column, a, Me.TableList(index), 1, False,
                                            Mi1)
                                        'Increament of Index.
                                        Mi1 += 1
                                ...
                                Else
                                    ...
                                    Try
                                        ...
                                        Catch t As Exception
                                            Log(t)
                                        End Try
                                        'Construction of New Draw Brwon Minster.
                                        MinisterOnTable(Mi2) = New DrawMinister(Row, Column, a, Me.TableList(index), -1, False,
                                            Mi2)
                                        'Increament Index.
                                        Mi2 += 1
                            ...
                            ElseIf System.Math.Abs(Me.TableList(index)(Row, Column)) = 6 Then
                                ...
                                If a = Color.Gray Then
                                    ...
                                    Try
                                        ...
                                        KingOnTable(Ki1) = New DrawKing(Row, Column, a, Me.TableList(index), 1, False,
                                            Ki1)
                                        'Increament of Index.
                                        Ki1 += 1
                                ...
                                Else
                                    ...
                                    Try
                                        ...
                                        KingOnTable(Ki2) = New DrawKing(Row, Column, a, Me.TableList(index), -1, False,
                                            Ki2)
                                        'Increament of Index.
                                        Ki2 += 1
        ...
        Public Function HuristicKish(AAllDraw As List(Of AllDraw), a As Color, ij As Integer, ByRef Less As Double, Order As Integer) As Integer(,)
            ...
            If AAllDraw.Count > 0 Then
                    ...
                    While k < AllDraw.SodierMovments
                        ...
                        While SolderesOnTable(i) IsNot Nothing AndAlso SolderesOnTable(i).SoldierThinking(k) IsNot Nothing AndAlso j < SolderesOnTable(i).SoldierThinking(k).TableListSolder.Count
                            Try
                                ...
                                If SolderesOnTable(i).SoldierThinking(k).HuristicListSolder(j)(0) + SolderesOnTable(i).SoldierThinking(k).HuristicListSolder(j)(1) + SolderesOnTable(i).SoldierThinking(k).HuristicListSolder(j)(2) + SolderesOnTable(i).SoldierThinking(k).HuristicListSolder(j)(3) >= Less Then
                                    ...
                                    If True Then
                                        ...
                                        If (New ChessRules(1, TableS, Order).AchmazKingMove(Order, TableS, False)) AndAlso Not AllDraw.NoTableFound Then
                                            'If Order is Gray.
                                            If Order = 1 Then
                                    ...
                                    End If
                                    If Order = 1 Then
                                        AllDraw.SyntaxToWrite = ChessRules.CreateStatistic(Table, FormRefrigtz.MovmentsNumber, 1, SolderesOnTable(i).SoldierThinking(k).RowColumnSoldier(j)(1), SolderesOnTable(i).SoldierThinking(k).RowColumnSoldier(j)(0), Hit,
                                    ...
                                    SolderesOnTable(i).ConvertOperation(SolderesOnTable(i).SoldierThinking(k).RowColumnSoldier(j)(0), SolderesOnTable(i).SoldierThinking(k).RowColumnSoldier(j)(1), a, SolderesOnTable(i).SoldierThinking(k).TableListSolder(j), Order, False,
                                        i)
                    ...
                    While k < AllDraw.ElefantMovments
                        ...
                        While ElephantOnTable(i) IsNot Nothing AndAlso ElephantOnTable(i).ElefantThinking(k) IsNot Nothing AndAlso j < ElephantOnTable(i).ElefantThinking(k).TableListElefant.Count
                            Try
                                ...
                                'For Higher Huristic Values.
                                If ElephantOnTable(i).ElefantThinking(k).HuristicListElefant(j)(0) + ElephantOnTable(i).ElefantThinking(k).HuristicListElefant(j)(1) + ElephantOnTable(i).ElefantThinking(k).HuristicListElefant(j)(2) + ElephantOnTable(i).ElefantThinking(k).HuristicListElefant(j)(3) >= Less Then

                                    'Initiate Table of Current Object.
                                    Dim TableS As Integer(,) = ElephantOnTable(i).ElefantThinking(k).TableListElefant(j)
                                    If True Then
                                        'Achamaz Kish Mate of Current Table.
                                        If (New ChessRules(2, TableS, Order).AchmazKingMove(Order, TableS, False)) AndAlso Not AllDraw.NoTableFound Then
                                            'If Order is Gray.
                                            If Order = 1 Then
                                                If ChessRules.KishGrayAchmaz AndAlso AllDraw.DeptFirstSearch Then
                                                    i += 1
                                    ...
                                    End If
                                    If Order = 1 Then
                                        AllDraw.SyntaxToWrite = ChessRules.CreateStatistic(Table, FormRefrigtz.MovmentsNumber, 2, ElephantOnTable(i).ElefantThinking(k).RowColumnElefant(j)(1), ElephantOnTable(i).ElefantThinking(k).RowColumnElefant(j)(0), Hit,
                    ...
                    While k < AllDraw.HourseMovments
                        ...
                        While HoursesOnTable(i) IsNot Nothing AndAlso HoursesOnTable(i).HourseThinking(k) IsNot Nothing AndAlso j < HoursesOnTable(i).HourseThinking(k).TableListHourse.Count
                            Try
                                ...
                                'For Higher Huristic Values.
                                If HoursesOnTable(i).HourseThinking(k).HuristicListHourse(j)(0) + HoursesOnTable(i).HourseThinking(k).HuristicListHourse(j)(1) + HoursesOnTable(i).HourseThinking(k).HuristicListHourse(j)(2) + HoursesOnTable(i).HourseThinking(k).HuristicListHourse(j)(3) >= Less Then
                                    'Initiate Table of Current Object.
                                    Dim TableS As Integer(,) = HoursesOnTable(i).HourseThinking(k).TableListHourse(j)
                                    If True Then
                                        If True Then
                                            'Achamaz Kish Mate of Current Table.
                                            If (New ChessRules(3, TableS, Order).AchmazKingMove(Order, TableS, False)) AndAlso Not AllDraw.NoTableFound Then
                                                'If Order is Gray.
                                                If Order = 1 Then
                                                    If ChessRules.KishGrayAchmaz AndAlso AllDraw.DeptFirstSearch Then
                                                        i += 1
                                    ...
                                    End If
                                    If Order = 1 Then
                                        AllDraw.SyntaxToWrite = ChessRules.CreateStatistic(Table, FormRefrigtz.MovmentsNumber, 3, HoursesOnTable(i).HourseThinking(k).RowColumnHourse(j)(1), HoursesOnTable(i).HourseThinking(k).RowColumnHourse(j)(0), Hit,
                    ...
                    While k < AllDraw.BridgeMovments
                        ...
                        While BridgesOnTable(i) IsNot Nothing AndAlso BridgesOnTable(i).BridgeThinking(k) IsNot Nothing AndAlso j < BridgesOnTable(i).BridgeThinking(k).TableListBridge.Count
                            Try
                                ...
                                End If
                                'For Higher Huristic Values.
                                If BridgesOnTable(i).BridgeThinking(k).HuristicListBridge(j)(0) + BridgesOnTable(i).BridgeThinking(k).HuristicListBridge(j)(1) + BridgesOnTable(i).BridgeThinking(k).HuristicListBridge(j)(2) >= Less Then
                                    'Initiate Table of Current Object.
                                    Dim TableS As Integer(,) = BridgesOnTable(i).BridgeThinking(k).TableListBridge(j)
                                    If True Then
                                        'Achamaz Kish Mate of Current Table.
                                        If (New ChessRules(4, TableS, Order).AchmazKingMove(Order, TableS, False)) AndAlso Not AllDraw.NoTableFound Then
                                            'If Order is Gray.
                                            If Order = 1 Then
                                                If ChessRules.KishGrayAchmaz AndAlso AllDraw.DeptFirstSearch Then
                                                    i += 1
                                    ...
                                    End If
                                    If Order = 1 Then
                                        AllDraw.SyntaxToWrite = ChessRules.CreateStatistic(Table, FormRefrigtz.MovmentsNumber, 4, BridgesOnTable(i).BridgeThinking(k).RowColumnBridge(j)(1), BridgesOnTable(i).BridgeThinking(k).RowColumnBridge(j)(0), Hit,
                    ...
                    While k < AllDraw.MinisterMovments
                        ...
                        While MinisterOnTable(i) IsNot Nothing AndAlso MinisterOnTable(i).MinisterThinking(k) IsNot Nothing AndAlso j < MinisterOnTable(i).MinisterThinking(k).TableListMinister.Count
                            Try
                                'If there is Penalty Situation Continue.
                                If FormRefrigtz.OrderPlate = Order Then
                                    If MinisterOnTable(i).MinisterThinking(k).PenaltyRegardListKing(j).IsPenaltyAction() = 0 Then
                                        Less = -200000000
                                        i += 1
                                        Continue While
                                    End If
                                End If
                                'For Higher Huristic Values.
                                If MinisterOnTable(i).MinisterThinking(k).HuristicListMinister(j)(0) + MinisterOnTable(i).MinisterThinking(k).HuristicListMinister(j)(1) + MinisterOnTable(i).MinisterThinking(k).HuristicListMinister(j)(2) + MinisterOnTable(i).MinisterThinking(k).HuristicListMinister(j)(3) > Less Then
                                    'Initiate Table of Current Object.
                                    Dim TableS As Integer(,) = MinisterOnTable(i).MinisterThinking(k).TableListMinister(j)
                                    If True Then
                                        If True Then
                                            'Achamaz Kish Mate of Current Table.
                                            If (New ChessRules(5, TableS, Order).AchmazKingMove(Order, TableS, False)) AndAlso Not AllDraw.NoTableFound Then
                                                'If Order is Gray.
                                                If Order = 1 Then
                                                    If ChessRules.KishGrayAchmaz AndAlso AllDraw.DeptFirstSearch Then
                                                        i += 1
                                    ...
                                    Else
                                        If ChessRules.KishBrownAchmaz AndAlso Not AllDraw.DeptFirstSearch Then
                                            ...
                                            APredict.TableList.Add(TableS)
                                            APredict.SetRowColumn(0)
                                            If Nothing Is APredict.InitiatePerdictKish(APredict.MinisterOnTable(i).Row, CType(APredict.MinisterOnTable(i).Column, Integer), B, TableS, Order, False) Then
                                                i += 1
                                                Continue While
                                    ...
                                    End If
                                    If Order = 1 Then
                                        AllDraw.SyntaxToWrite = ChessRules.CreateStatistic(Table, FormRefrigtz.MovmentsNumber, 5, MinisterOnTable(i).MinisterThinking(k).RowColumnMinister(j)(1), MinisterOnTable(i).MinisterThinking(k).RowColumnMinister(j)(0), Hit,
                    ...
                    While k < AllDraw.KingMovments
                        ...
                        While KingOnTable(i) IsNot Nothing AndAlso KingOnTable(i).KingThinking(k) IsNot Nothing AndAlso j < KingOnTable(i).KingThinking(k).TableListKing.Count
                            Try
                                ...
                                End If
                                'For Higher Huristic Values.
                                If KingOnTable(i).KingThinking(k).HuristicListKing(j)(0) + KingOnTable(i).KingThinking(k).HuristicListKing(j)(1) + KingOnTable(i).KingThinking(k).HuristicListKing(j)(2) + KingOnTable(i).KingThinking(k).HuristicListKing(j)(3) >= Less Then
                                    'Initiate Table of Current Object.
                                    Dim TableS As Integer(,) = KingOnTable(i).KingThinking(k).TableListKing(j)
                                    If True Then
                                        'Achamaz Kish Mate of Current Table.
                                        If (New ChessRules(6, TableS, Order).AchmazKingMove(Order, TableS, False)) AndAlso Not AllDraw.NoTableFound Then
                                            'If Order is Gray.
                                            If Order = 1 Then
                                                If ChessRules.KishGrayAchmaz AndAlso AllDraw.DeptFirstSearch Then
                                                    i += 1
                                    ...
                                    End If
                                    If Order = 1 Then
                                        AllDraw.SyntaxToWrite = ChessRules.CreateStatistic(Table, FormRefrigtz.MovmentsNumber, 6, KingOnTable(i).KingThinking(k).RowColumnKing(j)(1), KingOnTable(i).KingThinking(k).RowColumnKing(j)(0), Hit,
                ...
                End While
            End If
            'If There is AAllDraw Movments Return Table.
            If Act Then
                Return Table
            End If
            'What Kind Of Table.
        ...
        Public Function InitiatePerdictKish(ii As Integer, jj As Integer, a As Color, Table As Integer(,), Order As Integer, TB As Boolean) As Integer(,)
            ...
            End While
            'Kish Consideration.
            If New ChessRules(1, TablInitKish, Order).Kish(TablInitKish, Order) Then
            ...
            While i < 2
                ...
                If iiii < 16 Then
                    Me.InitiateForEveryKindThingHome(New AllDraw(THIS), CType(CType(SolderesOnTable([In]).Row, Integer), Integer), CType(CType(SolderesOnTable([In]).Column, Integer), Integer), a, TablInit, Order,
                        False, [In])
                Else
                    Me.InitiateForEveryKindThingHome(New AllDraw(THIS), CType(1, Integer), 2, a, TablInit, Order,
                        False, [In])
                End If
                ...
                If AAllDraw.Count > 0 Then
                    ...
                    Tab = HuristicKish(AAllDraw, a, i, Less, Order)
                End If
                'Table is Not Found.
        ...
        Public Function InitiatePerdictKishEnemy(ii As Integer, jj As Integer, a As Color, Table As Integer(,), Order As Integer, TB As Boolean) As Boolean
            ...
            While i < 1
                ...
                Me.InitiateForEveryKindThingHome(New AllDraw(THIS), CType(SolderesOnTable([In]).Row, Integer), CType(SolderesOnTable([In]).Column, Integer), a, Table, Order,
                    False, [In])
