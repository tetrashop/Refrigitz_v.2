...
// Open spreadsheet for reading and writing
CSpreadSheet::CSpreadSheet(CString File, CString SheetOrSeparator,BOOL XLSXA, bool Backup) :
...
m_bAppend(false), m_bBackup(Backup), m_bTransaction(false)
{
	...
	if(XLSX)
	{
	m_stempString = m_sFile.Right(5);
	m_stempString.MakeLower();
	
	if (m_stempString == ".xlsx") // File is an Excel spreadsheet
	{
		m_bExcel = true;
	...
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		//m_sDsn.Format("DRIVER={%s};DSN='';FIRSTROWHASNAMES=1;READONLY=FALSE;CREATE_DB=\"%s\";DBQ=%s;Extended Properties='Excel 8.0;IMEX=1;HDR=NO'", m_sExcelDriver, m_sFile, m_sFile);
		//m_sDsn.Format("Provider=Microsoft.ACE.OLEDB.12.0;data source="+File+";Extended Properties=\"Excel 12.0 Xml;HDR=YES\"");
		//m_sDsn.Format("Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + m_sFile + ";Jet OLEDB:Engine Type=5;" +  "Extended Properties='Excel 8.0;IMEX=1;HDR=NO'");
...
bool CSpreadSheet::AddRow(CStringArray &RowValues, long row, bool replace)
{
	...
	if (row)
	{
		if (row <= m_dTotalRows) // Not adding new rows
		{
			...
			//	for(int i=0;i<m_aRows.GetCount();i++)
			//	//MessageBox(NULL,m_aRows.GetAt(i),0,0);
				m_dTotalRows++;
			}
...
bool CSpreadSheet::Commit()
{
	if (m_bExcel) // If file is an Excel spreadsheet
	{
		...
		if (m_bAppend)
		{
			...
			{
				m_sLastError = e->m_strError;
				//MessageBox(0,L"ExecuteSQL in Commit Cause to \'" +m_sLastError+L"\'",0,0);
				m_Database->Close();
				return false;
			}
		...
		catch(CDBException *e)
		{
			m_sLastError = e->m_strError;
			CString No;
		    No.Format(L"%d",m_aFieldNames.GetCount());
		    //MessageBox(0,L"ExecuteSQL in Commit Cause to \'" +m_sLastError+L"\' '\No="+No+L"\'.",0,0);
			m_Database->Close();
			return false;
		}
		...
		for (int k = 1; k < m_dTotalRows; k++)
		{
			...
			for (int i = 0; i < m_atempArray.GetSize(); i++)
			{
				//MessageBox(0,m_aFieldNames.GetAt(i),0,0);
...
void CSpreadSheet::GetExcelDriver()
{
	...
	do
	{
		if(XLSX)
		{
		if( wcsstr( pszBuf, L"Exce" ) != 0&&wcsstr( pszBuf, L"xlsx" ) != 0 )
		{
			// Found !
		...
		{
		if( wcsstr( pszBuf, L"Exce" ) != 0&&wcsstr( pszBuf, L"xls" ) != 0 )
		{
			// Found !
			m_sExcelDriver = CString( pszBuf );
			break;
...
bool CSpreadSheet::SetTotalRows()
{
	try{
		m_dTotalRows=0;
		CString CellValue;
		this->ReadCell(CellValue,m_dTotalColumns);
		while(CellValue!=L"")
			{
			m_dTotalRows++;
			this->ReadCell(CellValue,1,m_dTotalRows);			
			}
		}
		catch(CException&e)
		{
...
bool CSpreadSheet::SetTotalColumns()
{
	SetTotalRows();
	try{
		m_dTotalColumns=0;
		CString CellValue;
		this->ReadCell(CellValue,m_dTotalColumns,0);
		for(int i=1;i<m_dTotalRows;i++)
		while(CellValue!=L"")
			{
			m_dTotalColumns++;
			this->ReadCell(CellValue,m_dTotalColumns,i);			
			}
		}
		catch(CException&e)
		{
