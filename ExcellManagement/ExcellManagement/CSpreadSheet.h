...
#ifndef _CSPREADSHEET_H
#define _CSPREADSHEET_H
#include "stdafx.h"
#include <odbcinst.h>
#include <afxdb.h>
...
{
public:
	CSpreadSheet(); // Open spreadsheet for reading and writing
	CSpreadSheet(CString File, CString SheetOrSeparator,BOOL XLSXA, bool Backup = true); // Open spreadsheet for reading and writing
	...
	inline CStringArray *GetFieldNames () {return &m_aFieldNames;} // retrive the m_aFieldNames row from spreadsheet
	inline long GetTotalRows() {return m_dTotalRows;} // Get total number of rows in  spreadsheet
	void SetFieldNames(CStringArray &FieldNamesD){m_aFieldNames.RemoveAll();m_aFieldNames.Copy(FieldNamesD);}
	inline short GetTotalColumns() {return m_dTotalColumns;} // Get total number of columns in  spreadsheet
	bool SetTotalRows(); // Set total number of rows in  spreadsheet
	bool SetTotalColumns();// Set total number of columns in  spreadsheet
...
protected:
	...
	BOOL XLSX;;//Determine Kind of file. If TRUE Is 2010 more and if FALSE is less.
};
