...
public static class GlobalMembersBenchmark
{
	/*
...
//C++ TO C# CONVERTER NOTE: 'extern' variable declarations are not required in C#:
	...
	public static void benchmark(Position current, istream @is)
	{
	  ...
	  GlobalMembersUcioption.Options["Hash"] = ttSize;
	  GlobalMembersUcioption.Options["Threads"] = threads;
	  ...
	  ulong nodes = 0;
	  std.chrono.milliseconds.rep elapsed = GlobalMembersBenchmark.now();
	  Position pos = new Position();

	  for (uint i = 0; i < fens.Count; ++i)
	  {
		  std.unique_ptr<std.deque<StateInfo>> states = new std.unique_ptr<std.deque<StateInfo>>(new std.deque<StateInfo>(1));
		  pos.set(fens[i], GlobalMembersUcioption.Options["UCI_Chess960"], states.back(), GlobalMembersThread.Threads.main());

		  cerr << "\nPosition: " << (int)i + 1 << '/' << fens.Count << "\n";

		  if (limitType == "perft")
		  {
			  nodes += GlobalMembersSearch.perft(pos, limits.depth * Depth.ONE_PLY);
		  }

		  else
		  {
			  limits.startTime = GlobalMembersBenchmark.now();
			  GlobalMembersThread.Threads.start_thinking(pos, states, limits);
			  GlobalMembersThread.Threads.main().wait_for_search_finished();
			  nodes += GlobalMembersThread.Threads.nodes_searched();
		  }
	  }

	  elapsed = GlobalMembersBenchmark.now() - elapsed + 1; // Ensure positivity to avoid a 'divide by zero'

	  GlobalMembersMisc.dbg_print(); // Just before exiting

	  cerr << "\n===========================" << "\nTotal time (ms) : " << elapsed << "\nNodes searched  : " << (int)nodes << "\nNodes/second    : " << 1000 * nodes / elapsed << "\n";
	}
}
