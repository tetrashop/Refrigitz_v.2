...
//ORIGINAL LINE: tmp = *p;
	  ...
	  public static Move pick_best(ExtMove begin, ExtMove end)
	  {
	...
	public static void MovePicker.score()
	{
	  ...
	  Stats<Value, false> history = pos.this_thread().history;
	  FromToStats fromTo = pos.this_thread().fromTo;

	  Stats<Value, true> cm = (ss - 1).counterMoves;
	  Stats<Value, true> fm = (ss - 2).counterMoves;
	  Stats<Value, true> f2 = (ss - 4).counterMoves;

	  Color c = pos.side_to_move();

	  foreach (var m in * this)
	  {
		  m.value = history[pos.moved_piece(m)][(int)GlobalMembersBenchmark.to_sq(m)] + (cm != null ? cm[pos.moved_piece(m)][(int)GlobalMembersBenchmark.to_sq(m)] : Value.VALUE_ZERO) + (fm != null ? fm[pos.moved_piece(m)][(int)GlobalMembersBenchmark.to_sq(m)] : Value.VALUE_ZERO) + (f2 != null ? f2[pos.moved_piece(m)][(int)GlobalMembersBenchmark.to_sq(m)] : Value.VALUE_ZERO) + fromTo.get(c, m);
	  }
	}
	...
	public static void MovePicker.score()
	{
	  ...
	  foreach (var m in * this)
	  {
		  if (pos.capture(m))
		  {
			  m.value = GlobalMembersPsqt.PieceValue[(int)Phase.MG, pos.piece_on(GlobalMembersBenchmark.to_sq(m))] - Value(GlobalMembersBenchmark.type_of(pos.moved_piece(m))) + Stats<Value, false>.Max;
		  }
		  else
		  {
			  m.value = history[pos.moved_piece(m)][(int)GlobalMembersBenchmark.to_sq(m)] + fromTo.get(c, m);
		  }
	  }
	}
}
...
//  MovePicker CopyFrom(MovePicker NamelessParameter);
  ...
  public MovePicker(Position p, Move ttm, Value th)
  {
	...
	stage = (int)Stages.PROBCUT;

	// In ProbCut we generate captures with SEE higher than the given threshold
	ttMove = ttm && pos.pseudo_legal(ttm) && pos.capture(ttm) && pos.see_ge(ttm, threshold + 1)? ttm : Move.MOVE_NONE;

	stage += (ttMove == Move.MOVE_NONE);
  }
  public MovePicker(Position p, Move ttm, Depth d, Square s)
  {
	  this.pos = new Position(p);
	...
	if (pos.checkers() != 0)
	{
		stage = (int)Stages.EVASION;
	}

	else if (d > Depth.DEPTH_QS_NO_CHECKS)
	{
		stage = (int)Stages.QSEARCH_WITH_CHECKS;
	}

	else if (d > Depth.DEPTH_QS_RECAPTURES)
	{
		stage = (int)Stages.QSEARCH_NO_CHECKS;
	}

	else
	{
		stage = (int)Stages.QSEARCH_RECAPTURES;
		recaptureSquare = s;
		return;
	}

	ttMove = ttm && pos.pseudo_legal(ttm) ? ttm : Move.MOVE_NONE;
	stage += (ttMove == Move.MOVE_NONE);
  }
  ...
  public MovePicker(Position p, Move ttm, Depth d, Search.Stack s)
  {
	  this.pos = new Position(p);
	  this.ss = s;
	...
	countermove = pos.this_thread().counterMoves[(int)pos.piece_on(prevSq)][(int)prevSq];
  ...
  case Stages.PROBCUT:
	...
	case Stages.CAPTURES_INIT:
		...
		endMoves = generate<GenType.CAPTURES>(pos, cur);
...
//C++ TO C# CONVERTER TODO TASK: C# does not allow fall-through from a non-empty 'case':
	case Stages.GOOD_CAPTURES:
		while (cur < endMoves)
		{
			move = GlobalMembersMovepick.pick_best(cur++, endMoves);
			if (move != ttMove)
			{
				if (pos.see_ge(move, Value.VALUE_ZERO))
				{
					return move;
				}
	...
	case Stages.COUNTERMOVE:
		++stage;
		move = countermove;
		if (move != Move.MOVE_NONE && move != ttMove && move != ss.killers[0] && move != ss.killers[1] && pos.pseudo_legal(move) && !pos.capture(move))
		{
			return move;
		}
	...
	case Stages.QUIET_INIT:
		cur = endBadCaptures;
		endMoves = generate<GenType.QUIETS>(pos, cur);
		score<GenType.QUIETS>();
		if (depth < 3 * Depth.ONE_PLY)
		{
			ExtMove * goodQuiet = std.partition(cur, endMoves, (ExtMove m) =>
			{
				return m.value > Value.VALUE_ZERO;
			});
			GlobalMembersMovepick.insertion_sort(cur, goodQuiet);
		}
		else
		{
			GlobalMembersMovepick.insertion_sort(cur, endMoves);
		}
		++stage;
...
//C++ TO C# CONVERTER TODO TASK: C# does not allow fall-through from a non-empty 'case':
	...
	case Stages.EVASIONS_INIT:
		cur = moves;
		endMoves = generate<GenType.EVASIONS>(pos, cur);
...
//C++ TO C# CONVERTER TODO TASK: C# does not allow fall-through from a non-empty 'case':
	...
	case Stages.QCAPTURES_1_INIT:
  case Stages.QCAPTURES_2_INIT:
		cur = moves;
		endMoves = generate<GenType.CAPTURES>(pos, cur);
		score<GenType.CAPTURES>();
		++stage;
...
//C++ TO C# CONVERTER TODO TASK: C# does not allow fall-through from a non-empty 'case':
  ...
  case Stages.QCAPTURES_2:
		...
		if (stage == (int)Stages.QCAPTURES_2)
			break;
		cur = moves;
		endMoves = generate<GenType.QUIET_CHECKS>(pos, cur);
		++stage;
...
//C++ TO C# CONVERTER TODO TASK: C# does not allow fall-through from a non-empty 'case':
	case Stages.QRECAPTURES:
		...
		{
			move = GlobalMembersMovepick.pick_best(cur++, endMoves);
			if (GlobalMembersBenchmark.to_sq(move) == recaptureSquare)
			{
				return move;
			}
		}
...
//ORIGINAL LINE: template<GenType>
  ...
  private void score<GenType>()
  {
	...
	foreach (var m in * this)
	{
		m.value = GlobalMembersPsqt.PieceValue[(int)Phase.MG, (int)pos.piece_on(GlobalMembersBenchmark.to_sq(m))] - Value(200 * GlobalMembersBenchmark.relative_rank(pos.side_to_move(), GlobalMembersBenchmark.to_sq(m)));
	}
  }
