...
//C++ TO C# CONVERTER NOTE: 'extern' variable declarations are not required in C#:
	...
	public static TimeManagement Time = new TimeManagement(); // Our global time management object
	  ...
	  public const double MaxRatio = 7.09; // When in trouble, we can step over reserved time with this ratio
	  public const double StealRatio = 0.35; // However we must not steal time from remaining moves over this ratio
...
//ORIGINAL LINE: template<TimeType T>
	  public static int remaining<TimeType T>(int myTime, int movesToGo, int ply, int slowMover)
	  {
		...
		return (int)(myTime * Math.Min(ratio1, ratio2)); // Intel C++ asks for an explicit cast
	  }
}
