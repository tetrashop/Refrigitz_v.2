...
//C++ TO C# CONVERTER TODO TASK: The implementation of the following method could not be found:
	//string engine_info(<bool Do>bool to_uci);
//C++ TO C# CONVERTER TODO TASK: The implementation of the following method could not be found:
	//void prefetch<bool Do>(object addr);
//C++ TO C# CONVERTER TODO TASK: The implementation of the following method could not be found:
	//void start_logger<bool Do>(string fname);

//C++ TO C# CONVERTER TODO TASK: The implementation of the following method could not be found:
	//void dbg_hit_on<bool Do>(bool b);
//C++ TO C# CONVERTER TODO TASK: The implementation of the following method could not be found:
	//void dbg_hit_on<bool Do>(bool c, bool b);
//C++ TO C# CONVERTER TODO TASK: The implementation of the following method could not be found:
	//void dbg_mean_of<bool Do>(int v);
//C++ TO C# CONVERTER TODO TASK: The implementation of the following method could not be found:
	//void dbg_print<bool Do>();


	public static std.chrono.milliseconds.rep now<bool Do>()
	{
...
/// In addition to the UCI ones, also some additional debug commands are supported.
	...
	public static void loop(int argc, string[] argv)
	{
	  ...
	  pos.set(StartFEN, false, States.back(), GlobalMembersThread.Threads.main());
	  ...
	  do
	  {
		  if (argc == 1 && !cmd = Console.ReadLine()) // Block here waiting for input or EOF
		  {
			  cmd = "quit";
		  }
		  ...
		  if (token == "quit" || token == "stop" || (token == "ponderhit" && GlobalMembersSearch.Signals.stopOnPonderhit != null))
		  {
			  GlobalMembersSearch.Signals.stop = true;
			  GlobalMembersThread.Threads.main().start_searching(true); // Could be sleeping
		  }
		  else if (token == "ponderhit")
		  {
			  GlobalMembersSearch.Limits.ponder = 0; // Switch to normal search
		  }
		  ...
		  else if (token == "uci")
		  {
			  ...
			  Console.Write(GlobalMembersMisc.engine_info(true));
			  Console.Write("\n");
			  Console.Write(GlobalMembersUcioption.Options);
			  Console.Write("\nuciok");
			  Console.Write("\n");
		  ...
		  else if (token == "ucinewgame")
		  {
			  ...
			  GlobalMembersTimeman.Time.availableNodes = 0;
		  }
		  ...
		  {
			  GlobalMembersUci.go(pos, @is);
		  }
		  else if (token == "position")
		  {
			  GlobalMembersUci.position(pos, @is);
		  }
		  else if (token == "setoption")
		  {
			  GlobalMembersUci.setoption(@is);
		  }
		  ...
		  {
			  GlobalMembersBenchmark.benchmark(pos, @is);
		  }
		  else if (token == "d")
		  {
			  Console.Write(SyncCout.IO_LOCK);
		  ...
		  {
			  Console.Write(SyncCout.IO_LOCK);
			  Console.Write(GlobalMembersEvaluate.trace(pos));
			  Console.Write("\n");
			  Console.Write(SyncCout.IO_UNLOCK);
		  }
		  else if (token == "perft")
		  {
			  ...
			  @is >> depth;
			  ss << GlobalMembersUcioption.Options["Hash"] << " " << GlobalMembersUcioption.Options["Threads"] << " " << depth << " current perft";

			  GlobalMembersBenchmark.benchmark(pos, ss);
		  }
		  else
		  {
			  Console.Write(SyncCout.IO_LOCK);
	  ...
	  GlobalMembersThread.Threads.main().wait_for_search_finished();
	}
...
///           use negative values for y.
	...
	public static string value(Value v)
	{
	  ...
	  if (Math.Abs(v) < Value.VALUE_MATE - MAX_PLY)
	  {
		  ss << "cp " << v * 100 / Value.PawnValueEg;
	  }
	  else
	  {
		  ss << "mate " << (v > 0 ? Value.VALUE_MATE - v + 1 : -Value.VALUE_MATE - v) / 2;
	  }
...
/// UCI::square() converts a Square to a string in algebraic notation (g1, a7, etc.)
	...
	{
	  return (string)({(sbyte)('a' + GlobalMembersBenchmark.file_of(s)), (sbyte)('1' + GlobalMembersBenchmark.rank_of(s))});
	}
...
/// castling moves are always encoded as 'king captures rook'.
	...
	public static string move(Move m, bool chess960)
	{
	  ...
	  if (GlobalMembersBenchmark.type_of(m) == MoveType.CASTLING && !chess960)
	  {
		  to = GlobalMembersBenchmark.make_square(to > ((int)from) != 0 ? File.FILE_G : File.FILE_C, GlobalMembersBenchmark.rank_of(from));
	  }

	  string move = GlobalMembersUci.square(from) + GlobalMembersUci.square(to);
	  if (GlobalMembersBenchmark.type_of(m) == MoveType.PROMOTION)
	  {
		  move += " pnbrqk"[(int)GlobalMembersBenchmark.promotion_type(m)];
	  }
...
//C++ TO C# CONVERTER TODO TASK: The implementation of the following method could not be found:
	...
	public static bool BestMove;
		...
		public static string MoveToFrom = "";
...
//C++ TO C# CONVERTER TODO TASK: The implementation of the following method could not be found:
	//  std::unique_ptr<std::deque<StateInfo>> States(new std::deque<StateInfo>(1));
	 ...
	 /*
	  ...
	  public static void position(Position pos, istringstream @is)
	  {
		...
		pos.set(fen, GlobalMembersUcioption.Options["UCI_Chess960"], States.back(), GlobalMembersThread.Threads.main());
	  ...
	  public static void setoption(istringstream @is)
	  {
		...
		{
			name += (string)(" ", string.IsNullOrEmpty(name) ? 0 : 1) + token;
		}

		// Read option value (can contain spaces)
		while (@is >> token != 0)
		{
			value += (string)(" ", string.IsNullOrEmpty(value) ? 0 : 1) + token;
		}
	  ...
	  public static void go(Position pos, istringstream @is)
	  {
		...
		limits.startTime = GlobalMembersBenchmark.now(); // As early as possible!
		...
		GlobalMembersThread.Threads.start_thinking(pos, States, limits);
	  }
}
...
public class ExtMove <bool DoTrace = false>
{
  public Move move;
  public Value value;
...
public class TimeManagement <bool Root = true>
{
	public void init(Search.LimitsType limits, Color us, int ply)
	{
    
	  int minThinkingTime = GlobalMembersUcioption.Options["Minimum Thinking Time"];
	  int moveOverhead = GlobalMembersUcioption.Options["Move Overhead"];
	  int slowMover = GlobalMembersUcioption.Options["Slow Mover"];
	  int npmsec = GlobalMembersUcioption.Options["nodestime"];
	  ...
	  if (npmsec != 0)
	  {
		  ...
		  {
			  availableNodes = npmsec * limits.time[(int)us]; // Time is in msec
		  }
    
		  // Convert from millisecs to nodes
		  limits.time[(int)us] = (int)availableNodes;
		  limits.inc[(int)us] *= npmsec;
		  limits.npmsec = npmsec;
	  }
    
	  startTime = limits.startTime;
	  optimumTime = maximumTime = Math.Max(limits.time[(int)us], minThinkingTime);
    
	  int MaxMTG = limits.movestogo != 0 ? Math.Min(limits.movestogo, MoveHorizon) : MoveHorizon;
	  ...
	  for (int hypMTG = 1; hypMTG <= MaxMTG; ++hypMTG)
	  {
		  ...
		  int hypMyTime = limits.time[(int)us] + limits.inc[(int)us] * (hypMTG - 1) - moveOverhead * (2 + Math.Min(hypMTG, 40));
	  ...
	  if (GlobalMembersUcioption.Options["Ponder"])
	  {
