 ...
 * Unable To Draw WebJobRefregitz.Timer  Content at Tow Picture Box*******************************************RS*****0.12**4**Managements and Cuation Programing**(+)
 * WebJobRefregitz.Timer Working Hardly.**********************************************************************RS*****0.12**4**Managements and Cuation Programing**(+)
...
namespace WebJobRefregitz
{
    ...
    public partial class FormRefrigtz : Form
    {
        ...
        public WebJobRefregitz.Timer TimerText = null;
        public WebJobRefregitz.Timer GrayTimer = null;
        public WebJobRefregitz.Timer BrownTimer = null;
        ...
        public FormRefrigtz(bool AllDra)
        {
            ...
            if (!AllDrawLoad)
            {
                BrownTimer = new WebJobRefregitz.Timer(false);
                GrayTimer = new WebJobRefregitz.Timer(false);
                TimerText = new WebJobRefregitz.Timer(true);
        ...
        private void SetTimer()
        {
            do
            {
                ...
                Thread.Sleep(100);
                WebJobRefregitz.Timer.AStarGreadyFirstSearch = RefrigtzDLL.AllDraw.AStarGreadyFirstSearch;
                WebJobRefregitz.Timer.AStarGreedytiLevelMax = RefrigtzDLL.AllDraw.AStarGreedyiLevelMax;
                WebJobRefregitz.Timer.UseDoubleTime = RefrigtzDLL.AllDraw.UseDoubleTime;
                WebJobRefregitz.Timer.StoreAllDrawCount = RefrigtzDLL.AllDraw.StoreADraw.Count;
            }
            while (true);
            ...
            */
            ...
            if (!exit)
            {
                ...
                if (!LoadAG)
                {
                    ...
                    a.AddRange(Process.GetProcessesByName("WebJobRefregitz"));
                    if (a.Count > 1)
                    {
                ...
                if (!AllDrawLoad)
                {
                    ...
                    {
                        GrayTimer = new WebJobRefregitz.Timer(true);
                        BrownTimer = new WebJobRefregitz.Timer(true);
                        GrayTimer.TimerInitiate();
                        BrownTimer.TimerInitiate();
                        if (OrderPlate == 1)
                        {
        ...
        //Painting of Form Refregitz PictureBox and Tow WebJobRefregitz.Timer Pictue Box on Time.
